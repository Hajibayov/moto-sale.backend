...
namespace moto.sale.user.backend.Services.Implementation
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public ResponseListTotal<EmployeeVM> GetAll(ResponseListTotal<EmployeeVM> response, CommonFilterVM filterVM)
        {
            int recordsToSkip = (filterVM.Page - 1) * filterVM.PageSize;

            var query = _employees.AllQuery.OrderByDescending(x => x.UpdatedAt != null ? x.UpdatedAt : x.CreatedAt);
            ...
            var db_query = query
                ...
                .ToList();
            response.Response.Data = _mapper.Map<List<EmployeeVM>>(db_query);
            return response;
        }
    }
}
