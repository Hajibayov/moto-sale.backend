    ...
    public class BlogService : IBlogService
    {
        ...
        public async Task<ResponseSimple> UpdateAsync(ResponseSimple response, BlogDto model, int id)
        {
            try
            {
                ...
                blog.CreatedBy = blogDb.CreatedBy;  
        ...
        public ResponseListTotal<BlogVM> GetAll(ResponseListTotal<BlogVM> response, CommonFilterVM filterVM)
        {
            int recordsToSkip = (filterVM.Page - 1) * filterVM.PageSize;

            var query = _blog.AllQuery.OrderByDescending(x => x.UpdatedAt != null ? x.UpdatedAt : x.CreatedAt);
            ...
            var db_query = query
                ...
                .ToList();
            response.Response.Data = _mapper.Map<List<BlogVM>>(db_query);
            return response;
        }
